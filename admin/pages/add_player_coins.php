...
$successMessage = getSession('admin_add_player_coins_success');
if ($successMessage !== false) {
    unsetSession('admin_add_player_coins_success');
    echo_success($successMessage);
}
        ...
        try {
            ...
            if (!$character) {
                ...
                if (!$account) {
                    ...
                    $db->exec("UPDATE `accounts` SET `{$coinColumn}` = `{$coinColumn}` + {$coinsToAdd} WHERE `id` = {$accountID}");
                    setSession('admin_add_player_coins_success', "You have added {$coinsToAdd} Tibia Coins ({$coinType}) to the account '{$account['name']}' (associated with character '{$characterName}').");
                        ...
                        <div class="box-body">
                            <form action="<?= $base ?>" method="post">
                                ...
                                <div class="form-group d-flex align-items-end">
                                    ...
                                    <div style="flex: 1;">
                                        <label for="player-coin-type">Tipo de coin</label>
                                        <select id="player-coin-type" name="coin_type" class="form-select">
                                            <option value="coins">Não transferível</option>
                                            <option value="transferable">Transferível</option>
                                        </select>
                                    </div>
                                </div>
